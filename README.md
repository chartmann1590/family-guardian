...
| GET    | `/api/circles/:id/health`    | ✓    | Family health snapshot — one row per member (battery, last-fix age, activity, paused, next routine, cached driving score, latest check-in) |
| GET    | `/api/circles/:circleId/members/:userId/timeline` | ✓ | Unified chronological feed merging visits, trips, check-ins, routine alerts, and other alert events; `?days=` ∈ [1, 30], `?before=<ms>` cursor, `?limit=` ∈ [10, 200] |
| GET    | `/api/places/:id/analytics`  | ✓    | Per-place visit analytics: per-member counts + dwell totals over `?days=` ∈ [1, 90] (default 30), plus week-over-week delta |
| GET    | `/api/circles/:circleId/digest/current` | ✓ | Latest persisted weekly digest snapshot for the circle |
| GET    | `/api/circles/:circleId/digest` | ✓ | List digest snapshots since `?since=<ms>` (default last 12 weeks) |
| PATCH  | `/api/users/me/digest-prefs` | ✓    | Toggle weekly-digest push delivery `{enabled: boolean}` (default OFF) |
| GET    | `/api/routine-templates`     | —    | List the 5 baked-in routine templates (school day, work commute, etc.) |
| POST   | `/api/users/me/routines/from-template` | ✓ | Instantiate `{templateId, placeId, customizations?}`; returns `{created, skipped}` — idempotent |
| GET    | `/api/users/me/emergency-contacts` | ✓ | List caller's emergency contacts (pending/accepted/revoked) |
| POST   | `/api/users/me/emergency-contacts` | ✓ | Invite by `{email}`; capped at 5 per user; pushes `emergency_contact_invite` to invitee |
| POST   | `/api/users/me/emergency-contacts/:id/respond` | ✓ | Contact accepts/revokes `{action: 'accept' \| 'revoke'}` |
| DELETE | `/api/users/me/emergency-contacts/:id` | ✓ | Soft-delete (sets status=`revoked`) — either party can revoke |
| GET    | `/api/users/me/pending-invites` | ✓ | Invites where caller is the contact and status is `pending` |
...
| GET    | `/ws`                         | ✓    | WebSocket upgrade (auth via cookie or `Authorization: Bearer`); emits `location_update`, `geofence_*` (with `notifyUserIds` for targeted dispatch), `sos_active` (with optional `source: 'user' \| 'crash'`), `sos_resolved`, `chat_message`, `reaction_added`, `reaction_removed`, `check_in`, `pause_changed`, `chat_typing`, `message_read`, `crash_pending`, `driving_score_updated`, `routine_deviation` (with `kind`: `missed_arrival` \| `overstay` \| `early_departure` \| `overstay_dwell` \| `curfew_violation`), `digest_ready`, `low_battery`, `emergency_contact_invite` |
...
Recently added (Sprint 6 — insights & visibility):
- ✅ **Family health widget** — `GET /api/circles/:id/health` returns a single roster of every member with battery %, last-fix age, current activity, paused flag, next upcoming routine, latest check-in, and a 5-minute-cached driving score. Renders as a horizontal "health pill" strip above the map on the PWA dashboard, the Android Map screen, and the iOS Map tab. No additional WS events — the widget refreshes on the existing `location_update` / `check_in` / `pause_changed` / `routine_deviation` / `sos_active` broadcasts.
- ✅ **Unified activity timeline** — new `GET /api/circles/:circleId/members/:userId/timeline?days=&before=&limit=` (`routes/timeline.js`) merges `visits` (started/ended), `trips` (started/ended), `check_ins`, `routine_alerts`, and other `alert_events` into one chronological feed via `UNION ALL`. Each item carries `{kind, at, payload}`. Row → JSON helpers extracted to `server/src/payloads.js`. Surfaced as a "Timeline" tab on the PWA member page and Android `MemberDetailScreen`, and as a "Timeline" section on the iOS member detail.
- ✅ **Driving score UI everywhere** — the long-existing `GET /api/users/:userId/driving-score` endpoint is now rendered with a color-coded score + 7/30/90 day range selector + component breakdown (hard brakes, speeding minutes, night driving %) on all three clients. Score also appears as a small chip on the PWA + Android dashboard avatars (sourced from the health endpoint's cached value).
- ✅ **Per-place visit analytics** — `GET /api/places/:id/analytics?days=30` returns per-member visit counts, total/average/longest dwell, last-visit timestamp, plus a week-over-week delta. Surfaced as a new "Analytics" tab on the PWA place modal, an Android `PlaceAnalyticsScreen` accessible by long-pressing a place, and a footer button on the iOS place detail.
- ✅ **Weekly family digest** — migration `024_digest_snapshots.sql` adds the `digest_snapshots` table (UNIQUE per `(circle_id, week_start)`) plus `alert_prefs.weekly_digest_enabled` (default OFF). `server/src/digest.js` aggregates per-member trip totals, top places by dwell, routine adherence, driving score, and check-in counts; a scheduler tick persists snapshots and pushes via existing `fanOut()` to opted-in members. `GET /api/circles/:circleId/digest/current` returns the latest snapshot; `PATCH /api/users/me/digest-prefs` toggles push delivery. Rendered as a "This week" card on the PWA dashboard (always visible) + Android `DigestScreen` + iOS Map tab. Retention: 12 weeks.

Recently added (Sprint 7 — iOS parity + routines v2 + safety v2):
- ✅ **iOS parity catch-up** — iOS now has dedicated `RoutinesScreen` (list / toggle / delete / manual-create with day-of-week chips + time picker + tolerance slider + dwell kind), a full Weekly Digest detail screen (this-week card on Map tab → `DigestScreen` with per-member breakdown + circle totals + past-12-week history), and a `TripsScreen` with range filter (7d/30d/90d) — closing the gap with Android.
- ✅ **Dwell-time routines** — migration `025_routines_dwell.sql` recreates `routines` with `kind IN ('arrival','departure','dwell')` and adds `expected_dwell_minutes`. Mining (`mineRoutines`) detects per-(user, place, day-of-week) dwell patterns from visits ≥ 5 min, derives median dwell + tolerance. Sweep (`evaluateRoutineSweep`) fires `overstay_dwell` when a still-open visit exceeds `expected + dwell + tolerance` (with its own 60-min firing window). New alert kind also lives in `routine_alerts`.
- ✅ **Routine templates** — `server/src/routineTemplates.js` ships five baked-in templates (School day, Work commute, After-school return, Weekend service, Nightly home-by). `GET /api/routine-templates` lists them; `POST /api/users/me/routines/from-template` instantiates one against a chosen place (returns `created` + `skipped` for idempotency). Templates picker appears on the PWA Settings page + Android RoutinesScreen + iOS RoutinesScreen.
- ✅ **Bedtime / curfew mode** — migration `026_curfew.sql` adds `alert_prefs.curfew_enabled / curfew_start / curfew_end / curfew_home_place_id` and changes `routine_alerts` UNIQUE to `(user_id, kind, fired_local_date)` so synthetic curfew rows coexist with routine rows. `server/src/curfew.js` `evaluateCurfewSweep` runs every 5 min from the scheduler — fires `curfew_violation` (one per night per user) when the user is NOT inside the configured home geofence during the curfew window. Honors pause + midnight wrap. Settings UI on PWA + Android + iOS.
- ✅ **Low-battery push alerts (opt-in, edge-triggered)** — migration `027_low_battery.sql` adds `alert_prefs.low_battery_alerts` (the existing `low_battery_threshold` is reused). `POST /api/locations` now runs `checkLowBattery`: when the subject's battery crosses below the most permissive watcher threshold (transitioning from above), `fanOutToUsers` pushes a `low_battery` event only to the subset of circle members who have opted in. Hysteresis at `threshold + 5` re-arms. Settings toggle + threshold picker on each client.
- ✅ **Emergency contacts (non-circle SOS escalation)** — migration `028_emergency_contacts.sql` adds the `emergency_contacts` table (UNIQUE per `(user_id, contact_user_id)`, status `pending`/`accepted`/`revoked`). New routes (`server/src/routes/emergencyContacts.js`): list, invite-by-email, respond (accept/revoke), delete, list-pending-invites. Capped at 5 contacts per user. SOS activate (`routes/sos.js`) additionally `fanOutToUsers` to accepted contacts with a stripped payload — push notification only, **no lat/lng** (consent is for being notified, not for location visibility). Settings UI shipped on PWA + iOS; Android UI is a Sprint 8 carryover (data layer + API client already in place — manage via PWA in the meantime).
